<template>
  <div class="header">
    <div class="wrapper">
      <div class="circuit-info">
        <div class="circuit-id">Circuit {{ circuit.number }}</div>
        <div class="circuit-name">{{ circuit.name }}</div>
      </div>
      <div class="user-info" @click="logOut()">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="32"
          height="32"
          viewBox="0 0 512 512"
        >
          <path
            fill="currentColor"
            d="M406.5 399.6c-19.1-46.7-65-79.6-118.5-79.6h-64c-53.5 0-99.4 32.9-118.5 79.6C69.9 362.2 48 311.7 48 256c0-114.9 93.1-208 208-208s208 93.1 208 208c0 55.7-21.9 106.2-57.5 143.6m-40.1 32.7c-32 20.1-69.8 31.7-110.4 31.7s-78.4-11.6-110.5-31.7c7.3-36.7 39.7-64.3 78.5-64.3h64c38.8 0 71.2 27.6 78.5 64.3zM256 512a256 256 0 1 0 0-512a256 256 0 1 0 0 512m0-272a40 40 0 1 1 0-80a40 40 0 1 1 0 80m-88-40a88 88 0 1 0 176 0a88 88 0 1 0-176 0"
          />
        </svg>
        <div class="user-name">{{ user?.firstname }}</div>
      </div>
    </div>
  </div>
</template>

<script setup>
import { computed } from 'vue';

const { user, circuits } = useSettings();
const supabase = useSupabaseClient();

const { data } = await supabase.from('trot_circuit').select('id, name, number');
if (data) {
  circuits.value = data;
}

const circuit = computed(() => {
  return circuits.value.find((e) => {
    return e.id == user.value.circuit;
  });
});

const logOut = () => {
  user.value = undefined;
  reloadNuxtApp();
};
</script>

<style lang="scss" scoped>
.header {
  flex-shrink: 0;
  color: white;
  .wrapper {
    width: 80%;
    display: flex;
    justify-content: space-between;
    align-items: center;
    height: 100%;
    margin: auto;
  }
  width: 100%;
  height: 80px;
  background-color: black;
}
</style>
